export class Loader {
    static show(message = 'LOADING...') {
        ...
        if (!loader) {
            ...
            loader.innerHTML = `
                <div class="relative w-12 h-12 border-2 border-surface-200">
                    <div class="absolute inset-0 border-t-2 border-surface-900 animate-spin"></div>
                </div>
                <p class="mt-6 text-surface-900 font-heading font-bold text-xs tracking-widest uppercase" id="global-loader-text">${message}</p>
            `;
